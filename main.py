...
def _load_bottom_line_v2_prompt():
    path = os.path.join(os.path.dirname(__file__), 'prompts', 'lane4_bottom_line_v2_prompt.txt')
    try:
        with open(path, encoding='utf-8') as f:
            return f.read().strip()
    except FileNotFoundError:
...
LANE4_BOTTOM_LINE_V2_PROMPT = _load_bottom_line_v2_prompt()
...
def _deep_dive_legacy():
    ...
    if BOTTOM_LINE_V2:
        # OOU schools have no swim label; use academic-fit framing
        _bl_oou_instruction = (
            "## Bottom Line\n"
